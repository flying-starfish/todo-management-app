...
# ボリュームも削除する場合（PostgreSQLのデータが完全に消えます！）
docker-compose -f docker-compose.prod.yml down -v
```

**⚠️ 重要**：
- `docker-compose down`: コンテナのみ削除、データは**保持**される
- `docker-compose down -v`: コンテナ**とボリューム**を削除、データは**完全に消去**される

### 9.2 データベースのバックアップ（推奨）

削除前にデータをバックアップする場合：

```bash
# PostgreSQLのダンプを作成
docker exec todo-db-prod pg_dump -U todouser todo_db > backup_$(date +%Y%m%d_%H%M%S).sql

# バックアップから復元する場合
docker exec -i todo-db-prod psql -U todouser todo_db < backup_20251207_120000.sql
```
...
**データベースの切り替え**：
- 本番環境: PostgreSQL（Docker volume）
- 開発環境: SQLite（コンテナ内）
- データは**別々に管理**されるため、環境を切り替えてもデータは混在しません
...
**A**: 本番環境シミュレートでは、はい。開発環境では、いいえ。

**本番環境シミュレート（PostgreSQL）**:
```bash
# ボリュームの確認
docker volume ls | grep todo
...
# コンテナを削除してもボリュームは残る
docker-compose -f docker-compose.prod.yml down
docker-compose -f docker-compose.prod.yml up -d  # データは残っている！

# ボリュームを削除する場合のみデータが消える
docker-compose -f docker-compose.prod.yml down -v
```
...
| 環境 | データベース | 永続化 | データの場所 |
|------|------------|-------|-----------|
| 開発環境 | SQLite | ❌ なし | コンテナ内 |
| 本番シミュレート | PostgreSQL | ✅ あり | Dockerボリューム |
| ローカル開発（非Docker） | SQLite | ✅ あり | `backend/db/todos.db` |
...
**データベースマイグレーション**:
- スキーマ変更時はマイグレーションが必要
- 現在は`init_db()`で自動テーブル作成
- 本番環境ではAlembic等のマイグレーションツール推奨

### Q6: 開発環境と本番環境でデータベースが異なる理由は？

**A**: それぞれの環境に最適化されているためです。
...
この違いを学ぶことで、実際の本番デプロイ時に適切なデータベースを選択できるようになります。
